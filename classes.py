...
def scheduleCollection():
     ...
     if o =="1":
         ...
         lst=[r.numberOfBags,r.specialLocker,r.deliveryAddress,r.collectionDate,r.reservationID,Database.c.name,Database.c.surname,Database.c.email,totalCost,
...
def changeAddress():
    ...
    lst = [Database.c.email,Database.c.password,Database.c.name,Database.c.surname,Database.c.phoneNumber,Database.c.address,Database.c.DOB]
...
def changeUserDetails(email):
    ...
    adminPanel()
def createNewUser():
    n = input("Is a new user Administrator(1) or Drver(2)?")
    if n =="1":
        ...
        print("\nTo create new Driver account provide the following details: ")
...
def assignDriver(email,ID):
     ...
     bList = [aList[indexID],aList[indexID+1],aList[indexID+2],aList[indexID-2],aList[indexID+3],aList[indexID-4]]
